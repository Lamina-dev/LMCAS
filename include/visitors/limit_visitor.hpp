...
class LimitVisitor : public lamina::detail::SymbolicVisitor {
    std::string var;
    ...
    LAMINA_API std::shared_ptr<const SymbolicNode> taylor_fallback(const std::shared_ptr<const SymbolicNode>& num, const std::shared_ptr<const SymbolicNode>& den, int max_order = 8);
    LAMINA_API std::shared_ptr<const SymbolicNode> simplify_and_eval_ratio(const std::shared_ptr<const SymbolicNode>& ratio_node);
    LAMINA_API std::pair<std::shared_ptr<const SymbolicNode>, int> find_leading_term(const std::shared_ptr<SymbolicExpr>& series_expr, const std::string& expand_var, const std::shared_ptr<SymbolicExpr>& expand_point, int max_order);
    LAMINA_API static int get_sign(const std::shared_ptr<const SymbolicNode>& node);

    /// --- Limits at infinity helpers ---
    LAMINA_API bool is_limit_at_infinity() const;
    LAMINA_API bool is_limit_at_neg_infinity() const;
    LAMINA_API int get_polynomial_degree(const std::shared_ptr<const SymbolicNode>& node) const;
    LAMINA_API std::shared_ptr<const SymbolicNode> get_leading_coefficient(const std::shared_ptr<const SymbolicNode>& node) const;
    LAMINA_API std::shared_ptr<const SymbolicNode> limit_rational_at_infinity(const std::shared_ptr<const SymbolicNode>& num, const std::shared_ptr<const SymbolicNode>& den);
    enum class GrowthClass { Constant, Logarithmic, Polynomial, Exponential, Unknown };
    LAMINA_API GrowthClass classify_growth(const std::shared_ptr<const SymbolicNode>& node) const;
    LAMINA_API int get_growth_polynomial_degree(const std::shared_ptr<const SymbolicNode>& node) const;
    LAMINA_API std::shared_ptr<const SymbolicNode> limit_by_growth_comparison(const std::shared_ptr<const SymbolicNode>& num, const std::shared_ptr<const SymbolicNode>& den);
    LAMINA_API std::shared_ptr<const SymbolicNode> handle_neg_infinity_limit(const std::shared_ptr<const SymbolicNode>& expr);
    LAMINA_API std::shared_ptr<const SymbolicNode> substitute_neg_t(const std::shared_ptr<const SymbolicNode>& node, const std::string& t_var) const;

    /// --- Direction helpers ---
    int determine_sign_near_point(const std::shared_ptr<const SymbolicNode>& expr, const std::string& dir);
    LAMINA_API std::shared_ptr<const SymbolicNode> select_branch_by_direction(const PiecewiseNode& node, const std::string& dir);
    LAMINA_API bool condition_satisfied_by_direction(const std::shared_ptr<const SymbolicNode>& condition, const std::string& dir);
    LAMINA_API std::optional<int> evaluate_relational_sign(const std::shared_ptr<const RelationalNode>& rel, const std::string& dir);
    LAMINA_API std::optional<std::shared_ptr<const SymbolicNode>> evaluate_sgn_limit(const std::shared_ptr<const SymbolicNode>& arg);
    LAMINA_API std::optional<std::shared_ptr<const SymbolicNode>> evaluate_abs_limit(const std::shared_ptr<const SymbolicNode>& arg);
...
public:
    ...
    void visit(const VariableNode& node) override { if (node.name() == var) result = point->clone(); else result = node.clone(); }
    ...
    void visit(const MatrixNode& node) override { result = node.clone(); }
    ...
    LAMINA_API void visit(const PiecewiseNode& node) override;
    void visit(const SummationNode& node) override { result = node.clone(); }
    void visit(const ProductNode_Op& node) override { result = node.clone(); }
    void visit(const TransformNode& node) override { result = node.clone(); }
    void visit(const QuantifierNode& node) override { result = node.clone(); }
    void visit(const SetBuilderNode& node) override { result = node.clone(); }
...
inline bool LimitVisitor::is_neg_inf(const std::shared_ptr<const SymbolicNode>& node) const {
    if (!node) return false;
...
inline std::optional<int> LimitVisitor::get_node_sign(const std::shared_ptr<const SymbolicNode>& node) const {
...
inline double LimitVisitor::get_numeric_value(const std::shared_ptr<const NumberNode>& num) const {
...
inline bool LimitVisitor::is_bounded(const std::shared_ptr<const SymbolicNode>& node) const {
    if (!node) return false;
 ...
 * 因此在极限点附近有界。
 */
inline bool LimitVisitor::is_bounded_expression(const std::shared_ptr<const SymbolicNode>& node) const {
    if (!node) return false;
...
inline bool LimitVisitor::tends_to_zero(const std::shared_ptr<const SymbolicNode>& node) const {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::eval_limit(const std::shared_ptr<const SymbolicNode>& expr) {
    EvaluationScope scope;
...
inline LimitVisitor::IndeterminateForm LimitVisitor::classify_product_form(const std::vector<std::shared_ptr<const SymbolicNode>>& vals) {
...
inline LimitVisitor::IndeterminateForm LimitVisitor::classify_power_form(const std::shared_ptr<const SymbolicNode>& bv, const std::shared_ptr<const SymbolicNode>& ev) {
...
inline LimitVisitor::IndeterminateForm LimitVisitor::classify_add_form(const std::vector<std::shared_ptr<const SymbolicNode>>& vals) {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::try_squeeze(const std::shared_ptr<const SymbolicNode>& expr) {
...
inline std::pair<std::shared_ptr<const SymbolicNode>, std::shared_ptr<const SymbolicNode>> LimitVisitor::extract_num_den(const std::shared_ptr<const SymbolicNode>& expr) {
    if (auto mul = std::dynamic_pointer_cast<const MultiplyNode>(expr)) {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::resolve_zero_times_inf(const std::vector<std::shared_ptr<const SymbolicNode>>& factors, const std::vector<std::shared_ptr<const SymbolicNode>>& factor_vals) {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::resolve_inf_minus_inf(const AddNode& node, const std::vector<std::shared_ptr<const SymbolicNode>>&) {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::resolve_exponential_form(const std::shared_ptr<const SymbolicNode>& base, const std::shared_ptr<const SymbolicNode>& exponent) {
...
inline std::shared_ptr<const SymbolicNode> LimitVisitor::apply_lhopital(const std::shared_ptr<const SymbolicNode>& num, const std::shared_ptr<const SymbolicNode>& den) {
    EvaluationScope scope;
    if (!scope) return nullptr;
...
inline int LimitVisitor::determine_sign_near_point(const std::shared_ptr<const SymbolicNode>& expr, const std::string& dir) {
...
inline void LimitVisitor::visit(const AddNode& node) {
...
inline void LimitVisitor::visit(const MultiplyNode& node) {
    /// Handle negative infinity: substitute x = -t and evaluate lim(t→+∞)
    if (is_limit_at_neg_infinity()) {
...
inline void LimitVisitor::visit(const PowerNode& node) {
    /// Handle negative infinity: substitute x = -t and evaluate lim(t→+∞)
    if (is_limit_at_neg_infinity()) {
...
inline void LimitVisitor::visit(const FunctionNode& node) {
    /// Handle negative infinity: substitute x = -t and evaluate lim(t→+∞)
