 ...
 * @brief 计算函数的渐近线.
 *
 * 垂直渐近线:分母为零且极限为 +/-infinity 的点.
 * 水平渐近线:x->+/-infinity 时的极限值.
...
/**
 * @brief 计算全微分 df = sum(partialf/partialxᵢ)*dxᵢ.
 *
 ...
 * @brief 计算反函数导数 (f-¹)'(point) = 1 / f'(f-¹(point)).
 *
 * The convenience overload 先求解 f(x) = point 得到第一个 x_0,再计算
 * 1/f'(x_0).若 f'(x_0) = 0 则返回 nullptr.
 *
 * @param[in] f     函数表达式
 * @param[in] var   变量名
 ...
 * @brief 求解反函数 f-¹(y),即求解方程 f(x) = y 关于 x 的所有解.
 *
 * The convenience overload 利用现有求解器将 f(var) - y = 0 求解,返回所有分支.
 *
 * @param[in] f   函数表达式
 * @param[in] var 变量名
 * @param[in] y   目标值表达式
...
/**
 * @brief 计算显式曲线 y = f(x) 的曲率 κ = |f''| / (1 + f'^2)^(3/2)
...
/**
 * @brief 计算参数曲线 (x(t), y(t)) 的曲率 κ = |x'y'' - y'x''| / (x'^2 + y'^2)^(3/2)
 ...
 *
 * 公式: S = 2pi integralₐᵇ |f(x)| * sqrt(1 + f'(x)^2) dx
 ...
 * @brief 计算曲线 y = f(x) 绕 x 轴旋转所得旋转面的面积.
 *
 * The convenience overload 先尝试符号积分,若失败则回退到数值积分.
 *
 * @param[in] f 曲线函数表达式 f(x)
 * @param[in] var 自变量名称
 * @param[in] a 积分下限
 ...
 *
 * 公式: S = 2pi integralₐᵇ |x| * sqrt(1 + f'(x)^2) dx
 * Checked API 只接受可精确求值的符号定积分;超出支持域返回
 * `CasErrc::Inconclusive`.
 */
