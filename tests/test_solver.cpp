...
int main() {
    ...
    auto sol1 = lamina::solve_finite_checked(expr1, "x").value();
    ...
    auto sol2 = lamina::solve_finite_checked(expr2, "x").value();
    EXPECT_TRUE(sol2.size() == 1, "3x - 9 has one solution");
    print_sols("3x - 9 = 0", sol2);

    auto expr3 = SymbolicExpr::add(x, nm5);
    auto sol3 = lamina::solve_finite_checked(expr3, "x").value();
    ...
    auto sol4 = lamina::solve_finite_checked(expr4, "x").value();
    ...
    auto sol5 = lamina::solve_finite_checked(expr5, "x").value();
