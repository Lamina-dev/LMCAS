...
static std::shared_ptr<SymbolicNode> make_num(int v) {
    return std::make_shared<NumberNode>(BigInt(v));
}

/// Create a NumberNode from a double
static std::shared_ptr<SymbolicNode> make_num_d(double v) {
    return std::make_shared<NumberNode>(static_cast<lmmc_real_t>(v));
}

/// Create a PowerNode expression
static SymbolicExpr make_power_expr(std::shared_ptr<SymbolicNode> base,
                                    std::shared_ptr<SymbolicNode> exponent) {
    SymbolicExpr expr;
    expr.root = std::make_shared<PowerNode>(std::move(base), std::move(exponent));
    return expr;
}
